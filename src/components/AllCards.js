...
const AllCards = ({allCardData , color, dropDown}) => {
    ...
    const temp = (color, name) => {
        if(color=='green'){
            ...
            let value = ((((data.CY_Green_POS_Cnt - data.PY_Green_POS_Cnt)/data.PY_Green_POS_Cnt)*100).toFixed(2));
            return value
        };
        if(color=='amber'){
            const data = allCardData.find(item => item.Program_Name === name)  
            let value = ((((data.CY_Amber_POS_Cnt - data.PY_Amber_POS_Cnt)/data.PY_Amber_POS_Cnt)*100).toFixed(2));
            return value

        };
        if(color=='red'){
            const data = allCardData.find(item => item.Program_Name === name)  
            let value = ((((data.CY_Red_POS_Cnt - data.PY_Red_POS_Cnt)/data.PY_Red_POS_Cnt)*100).toFixed(2));
            return value
